...
Adapted from loom v1: v2's ``Task.latch`` is a ``LoopBlock`` dataclass
(header/fuel/while_) rather than v1's raw dict; the annotation logic
here reads it via attribute access.
"""
...
def annotation(
    ...
    *,
    ...
    if show_loops and task.latch is not None:
        latch = task.latch
        bits = [f"{loop_mark} {arrow} {latch.header}"]
