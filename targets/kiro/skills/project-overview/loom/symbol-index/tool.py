...
def symbol_index(inp: SymbolIndexInput) -> SymbolIndexOutput:
    ...
    import tempfile
    # Symbol data is recomputed every run — the on-disk index is
    # purely a transport medium so the multi-MB structure never
    # crosses the loom graph. A fresh temp dir per run is fine.
    index_dir = Path(tempfile.mkdtemp(prefix="project-overview-symbols-"))
