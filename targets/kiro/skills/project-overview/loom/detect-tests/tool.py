"""Tool body for the `detect-tests` task (loomv2 native contract).
...
2. Otherwise the build-system family from detect-build, provenance
   "build-system"; detect-build's skill scan is reused as-is.

Maps the family to a fallback test command. Cached via the `cache`
skill under `<cache_prefix>/detect-tests`.
"""
...
from io_types import DetectTestsInput, DetectTestsOutput

_CACHE_SH = (
    Path.home() / ".kiro" / "skills" / "home" / "cache" / "scripts" / "cache.sh"
)
_CACHE_NAME = "detect-tests"
...
def detect_tests(inp: DetectTestsInput) -> DetectTestsOutput:
    ...
    return DetectTestsOutput.from_dict(doc)
