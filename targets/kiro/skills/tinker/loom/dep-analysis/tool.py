"""Tool body for the `dep-analysis` task (loomv2 native contract).
...
Also reports top-level imports with no manifest entry. Cached per
workspace+commit.
"""
...
from io_types import DepAnalysisInput, DepAnalysisOutput

_CACHE_FILE = "dep-analysis.yaml"
_MAX_DEPS = 200
_MAX_UNRESOLVED = 40
_MAX_USED_IN = 10
...
def dep_analysis(inp: DepAnalysisInput) -> DepAnalysisOutput:
    cached = _read_cache(inp.cache_dir, inp.cache_mode)
    if cached is not None:
        cached["cache_hit"] = True
        return DepAnalysisOutput.from_dict(cached)
    ...
    return DepAnalysisOutput.from_dict(doc)
