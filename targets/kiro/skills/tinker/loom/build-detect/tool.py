"""Tool body for the `build-detect` task (loomv2 native contract).
...
Results cached per workspace+commit.
"""
...
from io_types import BuildDetectInput, BuildDetectOutput

_CACHE_FILE = "build-detect.yaml"
...
def build_detect(inp: BuildDetectInput) -> BuildDetectOutput:
    ...
    if cached is not None and cached.get("override") == (inp.build_system_override or ""):
        ...
        return BuildDetectOutput.from_dict(cached)
    ...
    _write_cache(inp.cache_dir, inp.cache_mode, {**doc, "override": override})
    return BuildDetectOutput.from_dict(doc)
