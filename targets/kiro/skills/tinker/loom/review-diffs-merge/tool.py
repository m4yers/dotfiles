...
Aggregates three review-diffs reviewer outputs (SWE + two domain
experts) into a single verdict/findings/summary tuple:

* `verdict` = `approved` only when all three reviewer verdicts
  equal `approved`; otherwise `revise`.
* `findings` = concatenated union of all reviewer findings (in
  swe/d1/d2 order). Each `message` gets a `[<reviewer_role>]`
...
* `summary` = per-reviewer summaries joined under `### <role>`
  markdown headers in swe/d1/d2 order.
"""
...
def _reviewer_slots(
    ...
    """Return [(verdict, findings, summary, role), ...] in swe/d1/d2 order."""
    return [
