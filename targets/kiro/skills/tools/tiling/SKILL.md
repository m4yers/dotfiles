...
Pane-centric tiling manager. All commands operate on panes within
the current window — no window concept is exposed to callers.
Panes are identified by stable IDs (`%NNN`).
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh \
...
```bash
eval "$(~/.kiro/skills/tools/tiling/scripts/run-ttm.sh layout build)"
...
```bash
eval "$(~/.kiro/skills/tools/tiling/scripts/run-ttm.sh layout check)"
# → KIRO=%42  EDITOR=%44  CONSOLE=%45
```

Exits 0 with pane IDs if intact. Exits 1 with error message
if layout is wrong — caller decides what to do.

### layout reset

Kill all panes except the caller, returning to a single pane.

```bash
eval "$(~/.kiro/skills/tools/tiling/scripts/run-ttm.sh layout reset)"
# → KIRO=%42
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane split -d right -s 63%
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane split -t %42 -d below -s 10%
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane write -t $EDITOR 'ls -la'
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane write -t $EDITOR -n 'partial input'
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane read -t $EDITOR
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane read -t $EDITOR -n 30
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane list
# %42: aim
# %44: vim
# %45: bash
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane kill -t %44
```
...
```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane resize -t $EDITOR -d left -s 10
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh pane resize -t $CONSOLE -d up -s 3
```
...
Set the activity label in the status bar. Skills call this
when starting work so the user sees what's active.

```bash
~/.kiro/skills/tools/tiling/scripts/run-ttm.sh activity set "cr-review CR-12345"
```
