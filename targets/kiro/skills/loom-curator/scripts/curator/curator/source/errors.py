"""Handler error envelopes + the ``safe_handler`` decorator.

Every handler returns either a success dict (``ok: True`` plus
handler-specific fields) or a uniform error envelope. The
``safe_handler`` decorator catches exceptions and maps them to
canonical codes from ``HandlerErrorCode`` so downstream consumers
can pattern-match on stable strings.
"""
...
class HandlerErrorCode:
    ...
    # small — every time this fires the right fix is usually to add
    # a more specific mapping above.
...
class HandlerError(Exception):
    """Raised inside a handler when a failure cleanly maps to a
    code.

    Use this when the handler already knows which category applies;
    it is preferred over raising ``ValueError``/``FileNotFoundError``
    and letting the decorator guess.
    """
...
def error_envelope(handler: str, code: str, message: str,
...
def safe_handler(handler_name: str
    ...
    """Wrap a handler's ``handle()`` function with uniform error
    mapping.

    The wrapped function always returns a dict:

    - Success: the handler's own dict plus ``ok: True`` (added if
      the handler did not set it).
    ...
    - ``httpx.HTTPStatusError`` → ``NETWORK_HTTP_ERROR`` (includes
      the status code in ``details``).
    - ``httpx.ConnectError`` / ``httpx.RequestError`` (non-timeout)
      → ``NETWORK_CONNECT_ERROR``
    - ``subprocess.TimeoutExpired`` → ``EXTERNAL_TOOL_ERROR`` with
      ``details.kind = "timeout"``
    - ``subprocess.CalledProcessError`` → ``EXTERNAL_TOOL_ERROR``
      with ``details.returncode``
    - ``FileNotFoundError`` → ``FILE_NOT_FOUND``
    - ``ValueError`` → ``PARSE_ERROR`` (handlers should prefer
      raising ``HandlerError`` with a more specific code).
        ...
        def _wrapped(*args, **kwargs) -> dict:
            try:
                ...
                return error_envelope(
                    ...
                    f"{e.cmd[0] if e.cmd else 'subprocess'} timed out "
                    f"after {e.timeout}s",
                    {"kind": "timeout",
                     "timeout_s": e.timeout,
                ...
                stderr_tail = (
                    (e.stderr or "")[:500]
                    if isinstance(e.stderr, str) else None
                )
                return error_envelope(
                    handler_name,
                    HandlerErrorCode.EXTERNAL_TOOL_ERROR,
                    f"{e.cmd[0] if e.cmd else 'subprocess'} exited "
                    f"{e.returncode}",
                    {"kind": "nonzero",
                ...
                # Handlers should prefer
                # HandlerError(PARSE_ERROR, ...) with details; plain
                # ValueError is the fallback.
                return error_envelope(
                    handler_name,
            ...
            if not isinstance(result, dict):
                raise TypeError(
                    f"handler {handler_name!r} returned non-dict: "
                    f"{type(result)}"
                )
...
def reused_envelope(dest_rel: str, type_: str, basename: str) -> dict:
    """Return a success envelope for a source that already lives in
    the vault. Skips the write step; downstream tasks see ok: true
