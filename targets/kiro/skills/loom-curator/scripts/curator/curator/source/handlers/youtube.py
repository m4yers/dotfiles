"""YouTube handler — transcript + thumbnail acquisition.

yt-dlp drives both the metadata pull and the subtitle download.
Manual subtitles are tried first, with auto-generated as fallback.
The VTT cues are flattened into prose with ``[MM:SS]`` anchors so
the source.md is human-readable.
"""
...
#
# Signals flow from strongest (explicit title keyword) to weakest
# (duration + category). The goal is a cheap, deterministic default —
# the classifier task supersedes downstream.

_DURATION_LONG_FORM_S = 40 * 60   # 40 minutes
_TITLE_KEYWORDS_PODCAST = ("interview", "podcast", "in conversation")
_TITLE_KEYWORDS_TALK    = ("keynote", " talk", "conference",
                            "tedx", "ted talk")
...
def handle_youtube(url: str, wd: Path) -> dict:
    ...
    if _exists:
        return reused_envelope(dest_rel, "video", basename)
...
def _fetch_transcript(url: str, wd: Path) -> str:
    """Download subtitles (manual preferred, auto as fallback),
    return plain text with [MM:SS] anchors every ~30s.
    ...
    for flags in attempts:
        ...
        try:
            ...
            # This attempt failed — try the next flag combo. We
            # only raise SOURCE_UNAVAILABLE if both manual and auto
            # fail.
            continue
        vtts = list(sub_path.glob("*.vtt"))
        if vtts:
            return _vtt_to_anchored_text(
...
def _vtt_to_anchored_text(vtt: str) -> str:
    """Flatten a VTT transcript into plain text with [MM:SS]
    markers.

    Delegates cue parsing to ``webvtt-py``, which handles WEBVTT /
    NOTE / Kind / Language headers, cue settings, multi-line cue
    text, and timestamp normalization across comma vs dot
    separators. We then walk the cues in order and emit a [MM:SS]
    anchor each time a cue starts ``ANCHOR_EVERY_SECONDS`` past the
    previous anchor.
    """
    out: list[str] = []
    last_mark = -1
    for cap in webvtt.read_buffer(io.StringIO(vtt)):
        ...
        if bucket != last_mark // ANCHOR_EVERY_SECONDS:
            ...
            out.append(f"\n[{mm:02d}:{ss:02d}] ")
            last_mark = total_s
        # cue text may be multi-line; collapse to a single space
        # run so the anchored paragraph reads cleanly.
