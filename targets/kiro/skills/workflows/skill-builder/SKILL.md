...
```
~/.kiro/skills/
├── tools/         # Single-purpose utilities (editor, tiling, xlsx)
└── workflows/     # Multi-phase interactive skills (retro, skill-builder)
```
...
- **name** (optional for create, required for update):
  kebab-case name
- **category** (optional): `tools` or `workflows`
...
Ask about the skill's purpose, when it triggers, and which category
it belongs to.

**Constraints:**
- You MUST confirm the name is kebab-case
- You MUST determine the category
  (`tools`/`workflows`) — suggest one based on
  purpose
...
Place scripts in a `scripts/` subfolder within the skill directory.
...
After creating a skill, check if any subagent or prompt needs
alignment.
...
After the skill is created and reviewed with the user, run the
`skill-reviewer` skill against it.
...
| Status               | Criteria                             |
|----------------------|--------------------------------------|
| `DONE`               | Skill created/updated, reviewer pass |
| `DONE_WITH_CONCERNS` | Skill written but reviewer warnings  |
|                      | remain                               |
| `BLOCKED`            | Cannot determine category or name    |
| `NEEDS_CONTEXT`      | Purpose or trigger phrases not given |
