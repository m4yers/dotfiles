...
│   └── queries.md
├── schemas/              # Optional — JSON schemas for task
│   └── output.yaml       #   outputs and structured artifacts
...
(algorithms, merge procedures, select criteria, prose
playbooks). `schemas/` holds machine-validated JSON
schemas for task outputs and other structured artifacts —
keep them top-level so producer/consumer code can resolve
them without crossing into `references/`. `templates/`
holds files that are copied and filled in (artifact
templates, example outputs, stage templates). If a file
has placeholders or is used as a starting point, it
...
15. **TOC for long references** — reference files over
    ...
    with partial reads.
16. **No script re-narration** — sub-step prose MUST NOT
    restate what a script does internally (algorithm, edge
    cases, return values). A one-line purpose is fine; a
    detailed re-explanation is not. The agent reads the
    script directly when it needs more than the result.
    Re-narration drifts from the real script over time and
    bloats SKILL.md without adding decision-making content.
