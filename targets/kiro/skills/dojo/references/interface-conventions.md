...
Across all commands in the API, identical-looking flags MUST
mean the same thing. If `-t` accepts a target id in one
command, it MUST NOT accept a file path in another. If `--name`
is required on three commands, it MUST be optional only on
commands where the operation is over a default scope, with the
default explicitly documented.

Inconsistent flag semantics force callers to memorise
per-command quirks and break shell completion / scripting.
