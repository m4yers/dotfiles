...
def cmd_build(args):
    ...
    if len(panes) > 1:
        for p in panes:
            if p.pane_id != target:
                subprocess.run(
                    [TMUX_BIN, "kill-pane", "-t", p.pane_id],
                    capture_output=True,
                )

    raw = [TMUX_BIN, "split-window", "-hb", "-t", target,
           "-l", "50%", "-d", "-P", "-F", "#{pane_id}"]
...
def cmd_check(args):
    """Check if the standard 2-pane layout exists.
    ...
    panes = kiro.window.panes
    if len(panes) != 2:
        print(f"expected 2 panes, found {len(panes)}", file=sys.stderr)
        sys.exit(1)
    other = [p for p in panes if p.pane_id != target][0]
    if int(other.pane_left) >= int(kiro.pane_left):
