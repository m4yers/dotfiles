...
- transient  — /tmp/curator/* older than STALE_DAYS
"""
...
from engine import pages, vault, workdir
from engine.config import PAGE_STALE_DAYS
...
def lint(scope: str = "all") -> dict:
    ...
    if scope in ("all", "stale"):
        out["stale"] = _stale()
    if scope in ("all", "transient"):
        out["transient"] = workdir.list_stale()
    return out
