"""HTML article handler — fetch + image localization.

Trafilatura extracts the article body as markdown; embedded images
are downloaded into a sibling ``<basename>.assets/`` folder and the
markdown is rewritten to point at the local copies.
"""
...
def handle_html(url: str, wd: Path) -> dict:
    ...
    # Filename uses the natural title verbatim (with filesystem-
    # illegal chars stripped). Obsidian resolves
    # ``[[Title]]`` wikilinks against this filename directly —
    # no aliases needed. Author is recorded in frontmatter only.
    basename = vault.slugify_basename(title) or _host_slug(final_url) or "article"

    dest_rel, _exists = _html_dest(basename)
    if _exists:
        return reused_envelope(dest_rel, "article", basename)
    ...
    (wd / "meta.json").write_text(json.dumps(
        {"origin_url": url, "final_url": final_url,
         "images": image_log}, indent=2))
...
def _localize_images(md: str, base_url: str, assets_dir: Path,
                      wd: Path) -> tuple[str, list[dict]]:
    """Download every ``![]()`` image, rewrite src to local path,
    return log."""
    log = []
    urls_seen: dict[str, Path | None] = {}
    ...
    def _sub(match):
        ...
        if local is None:
            ...
            return match.group(0)
        log.append({"url": abs_url, "local": str(local.name),
                    "status": "ok"})
        # Wikilink works better in Obsidian:
        # ![[<basename>.assets/img.png]]
...
def _ext_from_content_type(ct: str) -> str | None:
    ...
    # Stdlib covers the common image MIME types and stays current
    # with additions; no hand-rolled map.
    return mimetypes.guess_extension(ct)


# ── basename / dest helpers ─────────────────────────────
