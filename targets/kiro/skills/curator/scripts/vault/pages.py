...
from vault.config import STUB_BODY_MIN

# __file__ = <skills>/home/curator/scripts/vault/pages.py
# .parent.parent → <skills>/home/curator/scripts/
_SCRIPTS_ROOT = Path(__file__).resolve().parent.parent
SCHEMA_DIR = _SCRIPTS_ROOT / "disk" / "schemas"
assert SCHEMA_DIR.is_dir(), f"SCHEMA_DIR missing: {SCHEMA_DIR}"
...
def materialize(approved_json_path: str, composed_json_path: str | None = None) -> dict:
    ...
    plan_data = {"plan": plan}
    plan_schema = json.loads((SCHEMA_DIR / "plan.schema.json").read_text())
    jsonschema.validate(instance=plan_data, schema=plan_schema)
